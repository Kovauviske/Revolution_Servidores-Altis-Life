...
switch (true) do
{
	...
	{
		if(vehicle player != player) exitWith {hint "Você não pode se curar dentro de um veículo..."};
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			player setDamage 0;
			player setFatigue 0;
			...
			closeDialog 0;
			hint "Os remédio funcionou. Você está com a vida cheia novamente"
		};
	};
	
	case (_item == "morphium"):
	{
		if(playerSide in [west,independent]) exitWith {hint "Você está em serviço!"};
		if(vehicle player != player) exitWith {hint "Você não pode se curar dentro de um veículo..."};
		if(([false,_item,1]call life_fnc_handleInv)) then
		{
			player setFatigue 1;
	...
	case (_item == "battery"):
	{
		...
		{
			life_battery = 100;
			hint "A sua bateria agora está em 100%";
		};
	};
	...
	case (_item == "redgull"):
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			...
			[] spawn
			{
				...
				titleText["Você está renovado, agora você tem 5 minutos de mais stamina","PLAIN"];
	...
	{
		if(!isNull life_spikestrip) exitWith {hint "Você já selecionou o spikeStrip"};
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
	...
	case (_item == "mauer"):
	{
		if(!isNull life_mauer) exitWith {hint "Você já colocou uma parede"};
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
	...
	case (_item == "fuelF"):
	{
		if(vehicle player != player) exitWith {hint "Você está abastecendo o veículo"};
	...
	case (_item == "methp"):
	{
		if(playerSide in [west,independent]) exitWith {hint "Não use drogas em serviço!"};
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
	...
	default
	{
		hint "Você não pode usar este item.";
	};
};
